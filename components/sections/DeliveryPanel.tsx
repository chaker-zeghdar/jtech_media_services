...
/**
 * The delivery block, minus its <Section /> wrapper.
 *
 * It used to be a section of its own. It is now the opening sub-block of
 * <Contact />, which is why this exports a bare <GoldPanel> rather than a
 * section: two visually distinct blocks inside one <Section id="contact">,
 * the same nesting <Featured /> uses for its ink spotlight.
 *
 * Nothing inside changed. The wilaya fee accordion in particular is real,
 * checkable information — grouped by fee band, straight from content/wilayas.ts
 * — and compressing it to fit a merge would have been a content regression
 * dressed up as simplification.
 *
 * It still reads the `delivery` message namespace, which is why that namespace
 * survives this pass even though no section is named after it any more.
 *
